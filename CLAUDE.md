...
- Ports carry real **latitude/longitude** — used directly by both `SeaMap` (static
  SVG, normalized to 0–1 viewBox) and `OSMMap` (real tiles via WebView+Leaflet).
  `MapContainer` wraps both with OSMMap as primary, SeaMap as fallback.
...
StatusPill, SeaMap, OSMMap, MapContainer, PassengerScreenHeader,
BangkeroScreenHeader, AuthErrorScreen
...
- ~~**3.5** — `SeaMap` SVG component.~~ **DONE.** Hand-fitted Bézier coastlines,
  ...
  flip inward past x > 0.62 so they never clip. `MapContainer` now wraps `OSMMap`
  (WebView+Leaflet, real tiles) as primary with `SeaMap` as fallback.
